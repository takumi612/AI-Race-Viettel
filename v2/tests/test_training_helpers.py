...
def _load_training(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "clinical_nlp_lab.training", module)
    spec.loader.exec_module(module)
    return module


def test_compute_non_o_metrics_reports_precision_recall_and_f1(monkeypatch):
    training = _load_training(monkeypatch)
...
def test_remove_nested_checkpoints_keeps_final_model(tmp_path, monkeypatch):
    training = _load_training(monkeypatch)
