...
1. Create a **private** Kaggle Dataset for the trained artifacts. Upload the
   ...
   `training_artifacts/ner_model/`, including `config.json` and
   `model.safetensors` (or the equivalent model weight files saved by the
   training run).
...
| Checkpoint members are missing | Rebuild the artifact Dataset from the complete original `results.zip`; verify it has `training_artifacts/ner_model/config.json` and `model.safetensors` before upload. Do not upload an extracted partial folder or an old `output.zip` instead. |
...
| GPU or vLLM error | Select a GPU in Notebook Settings, restart the session, and Run All. For out-of-memory or unsupported vLLM/CUDA combinations, choose a larger compatible GPU when available, close other GPU notebooks, or use the notebook's logged fallback path if it completes successfully. |
