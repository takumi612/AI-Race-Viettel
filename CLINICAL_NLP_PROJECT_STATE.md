...
- Notebook: `medical_information_extraction_lab.ipynb` chạy tuần tự thành công, 31 code cells, không lỗi.
- Test suite: 24/24 tests pass bằng `python -m unittest discover -s tests -v`.
- Inference: 100 input documents, 100 JSON output, `output.zip` hợp lệ 100 members, CRC/structure pass.
...
- Có relation baseline theo sentence/type/distance; relation chỉ nằm trong diagnostics khi schema submission chưa xác nhận.
...
Không có. Chỉ cần bổ sung annotation/schema chính thức nếu ban tổ chức cung cấp; khi đó có thể bật supervised XLM-R và chấm competition score thật.
...
- Mapping gate không đưa nhãn nội bộ chưa được xác nhận vào submission.
...
Entity, assertion và relation official labels chưa được cung cấp. Vì vậy không hard-code nhóm nội bộ thành official labels; output chính thức giữ đúng schema năm key và có thể rỗng an toàn, còn dự đoán nội bộ được lưu trong `diagnostics/`.
...
5. Stage 5 - assertion/context features, hybrid predictor, mapping gate: completed.
...
1. Thiếu train/validation annotation, official labels, ontology cho symptom/lab và competition schema; vì vậy supervised score và official competition score chưa thể tính.
...
5. Relation không đưa vào submission cho đến khi schema xác nhận hỗ trợ.
