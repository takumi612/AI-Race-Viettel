...
## D-015 - Mapping gate giữ submission schema-safe khi labels chưa xác nhận
...
**Chosen method:** Drop có kiểm soát entity không có official mapping, ghi diagnostics; không gán nhãn placeholder.  
**Reason:** Contract cấm hard-code official labels và workspace không có annotation/schema xác nhận.  
**Evidence:** `reports/stage_08_integration.json`: 100 JSON hợp lệ, 0 offset errors, ZIP 100 members/CRC pass, mapping status unconfirmed.  
**Alternatives rejected:** Xuất internal labels `DISEASE/DRUG/SYMPTOM` như official labels.  
**Impact:** Submission hiện là 100 mảng rỗng nhưng hợp lệ và trung thực; diagnostics vẫn giữ 845 internal entities.  
**Rollback plan:** Cập nhật `entity_type_mapping.json`/`assertion_mapping.json` sau organizer confirmation rồi rerun `tools/run_pipeline.py`.
